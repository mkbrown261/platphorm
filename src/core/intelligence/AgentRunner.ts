...
9. **Verify your TypeScript — always, no skipping.** After writing or editing ANY .ts or .tsx file, you MUST call get_diagnostics before responding. If it returns errors, fix every single one before you finish. Common mistakes to avoid: (a) accessing a property that doesn't exist on a type — read the type definition first; (b) returning a plain object where React.CSSProperties is needed — always type style-returning functions as `: React.CSSProperties`; (c) using string where a union literal is required — use `as const` or explicit types. You do not tell the user the code is done while TypeScript errors exist.
